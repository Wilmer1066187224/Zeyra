<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto mt-6 bg-white p-6 rounded shadow">
        <form action="{{ route('productos.update', $producto) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nombre" id="nombre" class="w-full border rounded p-2" value="{{ old('nombre', $producto->nombre) }}">
                @error('nombre') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>

            <div class="mb-4">
                <label for="codigo" class="block text-sm font-medium">Código (SKU)</label>
                <input type="text" name="codigo" id="codigo" class="w-full border rounded p-2" value="{{ old('codigo', $producto->codigo) }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" id="descripcion" class="w-full border rounded p-2">{{ old('descripcion', $producto->descripcion) }}</textarea>
            </div> 
    ...
    @error('categoria_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="precio" id="precio" class="w-full border rounded p-2" value="{{ old('precio', $producto->precio) }}">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="stock" id="stock" class="w-full border rounded p-2" value="{{ old('stock', $producto->stock) }}">
                @error('stock') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>

            <div class="mb-4">
                <label for="stock_minimo" class="block text-sm font-medium">Stock mínimo</label>
                <input type="number" name="stock_minimo" id="stock_minimo" class="w-full border rounded p-2" value="{{ old('stock_minimo', $producto->stock_minimo) }}">
